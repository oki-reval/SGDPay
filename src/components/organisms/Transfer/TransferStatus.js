...
const PinStatus = (props) => {
    ...
    return (
        <View style={styles.wrap}>
            <Icon name={`ios-${props.failed ? 'close' : 'checkmark'}-circle-outline`} style={{ color: props.failed ? color.failed : color.success, alignSelf: 'center' }} size={100} />
            <Text style={{fontSize: 16, fontWeight: 'bold', alignSelf: 'center', color: props.failed ? color.failed : color.success, marginBottom: 20}}>Transfer {props.failed? 'Gagal' : 'Berhasil'}</Text>
